...
const Requirements = React.forwardRef(
  ({ theEvent, translate, history }, ref) => {
    ...
    const isCancable = ![eventStates.FINISHED, eventStates.CANCELLED].includes(
      theEvent.status
    );
...
const CancelationPopup = ({ theEvent, hide, onCancelled }) => {
  ...
  return (
    <div>
      ...
      <Body>
        Are you sure you want to cancel? Please let us know the reason for
        canceling and if we can do anything better.
      </Body>
      ...
      <CheckboxTable
        ...
        onSave={setReason}
      />
      {chosenGig && chosenGig.offer && (
        <CancelationConsequences offer={chosenGig.offer} />
      )}
...
const CancelationConsequences = ({ offer }) => {
  ...
  if (isWithinCancelationPolicy) {
    return (
      <Body>
        Cancel now or within <b>{daysLeftInCancelationPolicy} days</b>, and
        receive a refund of <b>{bestCaseRefund.formatted}</b> otherwise you’ll
        recieve a refund of {<b>{worstCaseRefund.formatted}</b>}.
      </Body>
    );
  }
