...
class Button extends Component {
	...
	render() {
		const color = this.props.dangerous
			? "f44336"
		...
		var styles = {
			base: {
				...
				outline: "none",
				animation: this.props.isLoading
					? "rotating 2s 0.25s linear infinite"
					: null,
		...
		return (
			<div style={[styles.containerStyle, this.props.leftAlign && styles.left]}>
				<button
					disabled={
						this.props.disabled || this.props.isLoading || this.props.success
					}
					className={
						this.props.disabled || this.props.isLoading || this.props.success
							? "disabled"
				...
				>
					{" "}
					{this.props.isLoading ? null : this.props.success ? (
						<svg
							style={{ height: "32px" }}
							...
							viewBox="-30 0 230 200"
						>
							{" "}
							<path
								d="M 20,130 60,170 200,30"
								style={{
									stroke: "buttonFace",
									strokeWidth: "20",
									fill: "none"
