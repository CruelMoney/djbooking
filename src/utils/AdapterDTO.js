...
var user = {
	fromDTO: function(DTO) {
		return {
			...
			picture:
				!DTO.picture || DTO.picture.indexOf("default-profile-pic") !== -1
					? profilePic
					: DTO.picture,
			...
			settings: DTO.settings
				? settings.fromDTO(
						DTO.settings,
						DTO.app_metadata.isDJ,
						DTO.app_metadata.isCustomer
				  )
				: null,
			...
			censoredName:
				DTO.user_metadata.firstName +
				" " +
				DTO.user_metadata.lastName.slice(0, 1) +
				".",
			...
			provider: DTO.app_metadata.auth0Id
				? DTO.app_metadata.auth0Id.split("|")[0]
				: ""
		};
	},
...
var review = {
	fromDTO: function(DTO) {
		return {
			...
			author: DTO.reviewer
				? user.fromDTO(DTO.reviewer).censoredName
				: "Deleted user",
			authorPicture: DTO.reviewer
				? user.fromDTO(DTO.reviewer).picture
				: profilePic,
